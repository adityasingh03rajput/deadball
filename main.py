...
class Ball:
    def __init__(self, x, y, color):
        ...
        self.radius = 15
        self.velocity = [0, 0]
        self.health = 50  # Initial health is half of the player's max health
        self.max_health = 50
        self.frozen = 0  # Freeze timer
        self.random_target = (WIDTH // 2, HEIGHT // 2)  # Random target towards the center
    ...
    def move(self):
        ...
        # Move towards a random target near the center
        ...
        # If close to the target, set a new random target near the center
    ...
    def collide(self, other):
        ...
        if distance < self.radius + other.radius:
            ...
            # If colliding with a bullet, reduce health
            if isinstance(other, Bullet):
                self.health -= other.get_damage()
                if self.health <= 0:
                    self.frozen = 120  # Freeze for 2 seconds (60 FPS * 2)
...
while running:
    ...
    # Draw ball health bar
    pygame.draw.rect(screen, RED, (ball.x - 20, ball.y - 30, 40, 5))
    pygame.draw.rect(screen, GREEN, (ball.x - 20, ball.y - 30, int(40 * ball.health / ball.max_health), 5))
